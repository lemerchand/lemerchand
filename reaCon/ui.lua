...
function CLI:Parse(txt)
	...
	local trackStrEnd = trimmed:match('[<>=]')

	-- If we find an end set trackString
	-- and trim
	if trackStrEnd then 
		self.trackStr = trimmed:sub(1, trimmed:find(trackStrEnd)-1)
		trimmed = trimmed:sub(trimmed:find(trackStrEnd),-1)
	-- If not, then set it to the rest of the string
	elseif prefix and not trackStrEnd then 
		self.trackStr = trimmed
	end

	-- If the trackEnd was for arguments then
	if trackStrEnd == '=' then 
		self.args = trimmed
	else
		self.rop = trackStrEnd
	end
